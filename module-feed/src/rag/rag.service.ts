...
export class RagService {
    ...
    private async getQueryEmbedding(queryText: string): Promise<number[]> {
        const cachedEmbedding = this.embeddingCache.get(queryText);
        if (cachedEmbedding) {
            this.logger.log(`Using cached embedding for query: "${queryText.substring(0, 50)}..."`);
        ...
        try {
            ...
            if (realEmbedding && realEmbedding.length > 0) {
                ...
                this.embeddingCache.set(queryText, realEmbedding);
        ...
        this.embeddingCache.set(queryText, embedding);
        return embedding;
    }
    private async generateRealEmbedding(text: string): Promise<number[]> {
        ...
        try {
            ...
            // Match the preparation script format exactly
            const requestBody = {
                instances: [
                    {
                        content: text,
    ...
    private async findRelevantChunkIds(queryEmbedding: number[], maxChunks: number = 5): Promise<string[]> {
        ...
        try {
            ...
            const query = {
                datapoint: datapoint,
                neighborCount: Math.min(maxChunks * 2, 15), // Request more than we need to filter better
            };
