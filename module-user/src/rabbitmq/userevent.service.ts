...
import {UserCreatedEvent, UserDeletedEvent, UserUpdatedEvent} from '@/interfaces/user-events.interface';
...
export class UserEventsService implements OnApplicationBootstrap {
    ...
    private async connectWithRetry(retryCount = 0): Promise<void> {
        try {
            ...
            if (retryCount < this.maxRetries) {
                this.logger.warn(
                    `Failed to connect, retrying in ${this.retryDelay}ms... (Attempt ${retryCount + 1}/${this.maxRetries})`
                );
    ...
    private async publishWithRetry(exchange: string, routingKey: string, message: any, retryCount = 0): Promise<void> {
        try {
